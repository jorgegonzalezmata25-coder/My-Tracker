...
const FinDashboard=({state,allTx})=>{
  ...
  return(
    <div>
      <div className="fin-page-title" style={{fontFamily:FT.f1,fontSize:26,fontWeight:900,color:"#e8e4dc",marginBottom:4}}>Resumen Financiero</div>
      <div className="fin-page-sub" style={{fontFamily:FT.f2,fontSize:11,color:"#aaaacc",marginBottom:20,letterSpacing:1}}>{now.toLocaleString("es-MX",{month:"long",year:"numeric"}).toUpperCase()}</div>
        ...
        {[{label:"Ingresos",val:fmt(ing),color:FT.a2,icon:"↑"},{label:"Gastos",val:fmt(gas),color:FT.a4,icon:"↓"},{label:"Balance",val:fmt(bal),color:bal>=0?FT.a2:FT.a4,icon:"⊙"},{label:"Tasa ahorro",val:fmtP(sr),color:FT.a3,icon:"◈"},{label:"Deuda total",val:fmt(tDeu),color:FT.a4,icon:"⊗"},{label:"Total ahorrado",val:fmt(tAho),color:FT.a2,icon:"◎"}].map(k=>(
          <FCard key={k.label} style={{padding:16}}>
            <div style={{display:"flex",justifyContent:"space-between"}}>
              <div>
                <div className="fin-card-label" style={{fontFamily:FT.f2,fontSize:10,color:"#aaaacc",letterSpacing:1,textTransform:"uppercase",marginBottom:6}}>{k.label}</div>
                <div className="fin-card-value" style={{fontFamily:FT.f1,fontSize:20,fontWeight:700,color:k.color}}>{k.val}</div>
              </div>
      ...
      <div className="fin-widget-grid" style={{marginBottom:14}}>
        {catArr.length>0&&(
          <FCard>
            <div className="fin-section-title" style={{fontFamily:FT.f2,fontSize:11,color:"#aaaacc",letterSpacing:1,textTransform:"uppercase",marginBottom:14}}>Top gastos del mes</div>
            {catArr.map(([cat,amt])=>(
              <div key={cat} style={{marginBottom:12}}>
                <div style={{display:"flex",justifyContent:"space-between",marginBottom:4}}>
                  <span className="fin-bar-label" style={{fontSize:13,color:"#e8e4dc"}}>{cat}</span>
                  <span className="fin-bar-value" style={{fontSize:13,fontFamily:FT.f2,color:CAT_COLORS[cat]||FT.muted}}>{fmt(amt)}</span>
                </div>
        ...
        <FCard>
          <div className="fin-section-title" style={{fontFamily:FT.f2,fontSize:11,color:"#aaaacc",letterSpacing:1,textTransform:"uppercase",marginBottom:14}}>Últimos movimientos</div>
          ...
          {lastTx.map(t=>(
            <div key={t.id} style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"8px 0",borderBottom:"1px solid #1e1e30"}}>
              <div>
                <div className="fin-item-title" style={{color:"#e8e4dc",fontSize:13,fontFamily:FT.f3}}>{t.desc||t.category}</div>
                <div className="fin-item-sub" style={{color:CAT_COLORS[t.category]||"#aaaacc",fontSize:10,fontFamily:FT.f2}}>{t.category} · {t.date}</div>
              </div>
              <div className="fin-amount" style={{fontFamily:FT.f1,fontSize:15,fontWeight:700,color:t.type==="ingreso"?FT.a2:FT.a4}}>
      ...
      {budgets.length>0&&(
        <FCard>
          <div className="fin-section-title" style={{fontFamily:FT.f2,fontSize:11,color:"#aaaacc",letterSpacing:1,textTransform:"uppercase",marginBottom:14}}>Presupuestos</div>
...
const FIN_SIDEBAR_STYLE = `
  ...
  @media (min-width: 768px) {
    ...
    .fin-sidebar { display: flex; flex-direction: column; width: 200px; min-height: 100vh; background: #0d0d1a; border-right: 1px solid #1e1e30; position: sticky; top: 0; height: 100vh; overflow-y: auto; flex-shrink: 0; z-index: 20; }
    .fin-mobile-nav { display: none; }
    .fin-main { max-width: 100%; margin: 0; padding: 32px 36px 48px; flex: 1; overflow-y: auto; font-size: 15px; }
    .fin-topbar { display: none; }
    .fin-widget-grid { display: grid; grid-template-columns: 1fr 1fr; gap: 16px; }
    .fin-widget-grid-3 { display: grid; grid-template-columns: 1fr 1fr 1fr; gap: 16px; }
    .fin-widget-full { grid-column: 1 / -1; }
    .fin-widget-left { grid-column: 1 / 2; }
    .fin-widget-right { grid-column: 2 / 3; }
    .fin-main .fin-page-title { font-size: 32px !important; }
    .fin-main .fin-page-sub { font-size: 13px !important; }
    .fin-main .fin-card-label { font-size: 12px !important; }
    .fin-main .fin-card-value { font-size: 26px !important; }
    .fin-main .fin-section-title { font-size: 13px !important; }
    .fin-main .fin-item-title { font-size: 16px !important; }
    .fin-main .fin-item-sub { font-size: 12px !important; }
    .fin-main .fin-amount { font-size: 18px !important; }
    .fin-main .fin-pill { font-size: 12px !important; padding: 3px 10px !important; }
    .fin-main .fin-date { font-size: 12px !important; }
    .fin-main .fin-bar-label { font-size: 14px !important; }
    .fin-main .fin-bar-value { font-size: 14px !important; }
    .fin-main .fin-input label, .fin-main .fin-select label { font-size: 13px !important; }
    .fin-main .fin-input input, .fin-main .fin-select select { font-size: 15px !important; }
    .fin-main button { font-size: 14px !important; }
    .fin-main .fin-modal-title { font-size: 24px !important; }
  }
